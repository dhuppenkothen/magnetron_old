...
Most magnetars share common properties \citep[for general overviews of both phenomena, see ][]{woods2006,mereghetti2011}: slow spin periods between
...
data sets span thousands of such bursts. 
...
which allows us to extract a wealth of physically relevant time scales from SGR bursts. 
...
We extracted TTE data between $8 \, \mathrm{keV}$ and $200 \, \mathrm{keV}$ for each burst. The duration of a burst, $T_{90}$, is defined as the time in which the central $90\%$ of the photons, starting at $5\%$ and ending at $95\%$, reach the detector.  We extract data from this time interval and add $10\%$ of the $T_{90}$ duration on either side of start and end time to ensure the entire burst is within our data set. Due to their complex structure, defining exactly which temporal features in the light curve belong to a single burst, and which should be regarded as separate bursts, is 
...
p(\tau \given \mu_{\tau}) & = & e^{(\tau - \tau_{\mathrm{min}})/\mu_{\mathrm{\tau}}} \; ,
\end{eqnarray}
...
\ref{sec:priortest}, we give parameters and distributions for both priors.}
...
the amplitude $A$, and as derived quantities the fluence (as a proxy for total dissipated energy) and duration for model spikes for
 the whole \fermi/GBM sample of bursts from SGR J1550-5418. We can make inferences about individual parameters in the model by sampling
...
time scale as parametrised by the skewness parameter $s$, $\tau_{\mathrm{fall}} = s\tau$, the total dissipated energy $E$ and waiting time between consecutive spikes $t_\mathrm{wait}$. T
he exponential rise time scale and skewness for each model component are free parameters
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=9cm]{sgr1550_skewness_correlations.pdf}
...
spikes have faster rise time scales than decays. Moreover, we find that there is a positive correlation between the our flux measure and the skewness: brighter bursts have a higher
...
above $A/s \!\sim\! 100$, it seems to flatten out towards a slope of $0$ at lower fluxes: spikes with fluxes below that value have a similar ratio between rise and fall time scale. This 
...
In general, SOC processes follow a fractal geometry, an idea that was proposed by proponents of SOC theory early on \citep{bak1989}, and later
...
the fractal geometry can be characterised by the so-called Hausdorff fractal dimension $D_d$, which depends on the Euclidean space
...
magnetograms), this is not true for the $3$D Euclidean space, which is relevant for the geometry of both solar flares and magnetar bursts. 
...
For solar flares, there is a wealth of both imaging and timing studies suited to measure the relevant fractal dimensions. A large body of evidence suggests that area fractal dimension $D_2$ is
...
with a diffusions coefficient $\kappa$ and a diffusive spreading coefficient $\beta$.
...
Similarly, the differential distributions of burst duration, peak amplitude and total dissipated energy can be derived as
...
- for the first time model magnetar light curve variability in a probabilistic (procedural? reproducible? well-defined?) way \\

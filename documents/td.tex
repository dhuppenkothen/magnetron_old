...
In Figure \ref{fig:onespike}, we show the results for all four simulated bursts. Predictably, a spike with an amplitude below the background level is impossible for the model
...
much lower than the background itself. As soon as the spike has an amplitude larger than the background, the model behaves significantly different. 
For a signal with an amplitude of $\sim 3$ times background count rate, the posterior distribution of the position is peaked sharply at the position of the spike in the light curve, 
and only few samples are scattered throughout the rest of the light curve. Similarly, the posterior distribution of the amplitude clusters around $5$, with some samples at 
smaller amplitudes largely below the noise level. This scatter can be easily explained by the uncertainty in the amplitude introduced by the Poisson statistics. Additionally, the posterior distribution of the
number of spikes is extremely narrow at low values, with only very few samples indicating more than $3$ spikes in the model. 
Finally, raising the amplitude even further leads to an even stronger clustering in amplitude and position, indicating that the brightest spikes are easily detectable with our model 
